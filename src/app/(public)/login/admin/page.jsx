...
export default function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (result.success) {
      // AuthContext now has the user, cookie is set
      // Use setTimeout to let state settle, then redirect
      setTimeout(() => {
    ...
    <div className="min-h-screen flex items-center justify-center pt-16 pb-12 px-6">
      ...
      >
        <div className="text-center mb-10">
          ...
          <p className="text-xs text-muted tracking-widest uppercase mt-4">Admin Access</p>
        </div>

        <div className="bg-dark-card border border-dark-border p-8">
          <h1 className="text-2xl font-light text-light text-center mb-2">Admin Login</h1>
          <p className="text-sm text-muted text-center font-light mb-8">Password required</p>
